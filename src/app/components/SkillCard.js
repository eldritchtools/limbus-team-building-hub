...
export default function SkillCard({ skill, uptie, count = 0, level, type = "attack", index = 0 }) {
    ...
    return <div style={{ width: "100%", minWidth: "600px", height: "100%", display: "flex", flexDirection: "column", border: `1px ${affinityColorMapping[skillData.affinity]} solid`, borderRadius: "0.5rem", padding: "0.5rem", boxSizing: "border-box" }}>
        <div style={{ display: "flex", flexDirection: "row", justifyContent: "space-between", marginBottom: "0.25rem" }}>
            <div style={{ display: "flex", flexDirection: "row", gap: "0.25rem", alignItems: "center" }}>
                ...
                {count > 0 ? <div style={{ color: "#aaa", fontWeight: "bold", fontSize: "1.25rem" }}>x{count}</div> : null}
            </div>
            <div style={{ flex: "0 0 auto", color: "#aaa", fontWeight: "bold", fontSize: "1.25rem", marginLeft: "0.5rem" }}>
        ...
        <div style={{ display: "flex", flexDirection: "row", gap: "0.25rem", alignItems: "center", marginBottom: "0.25rem" }}>
            ...
            <span style={{ gap: "0" }}>
                {skillData.coinTypes.map((coin, i) => <Icon key={i} path={coin === "unbreakable" ? "unbreakable coin" : "coin"} style={{ height: "24px" }} />)}
            </span>
            {skillData.defType === "attack" || skillData.defType === "counter" ? <Icon path={"offense level"} style={{ width: "32px" }} /> : <Icon path={"defense level"} style={{ width: "32px" }} />}
            {level ?
